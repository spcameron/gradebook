...
class AttendanceStager:
    ...
    def staged_summary(
        ...
        """
        Summarize the number of staged changes by attendance status.
        ...
        Returns:
            dict[AttendanceStatus, int]: Counts of staged changes, keyed by AttendanceStatus.

        Notes:
            - This method mirrors the filtering logic in `pending()`, but instead of returning tuples, it aggregates counts by status.
            - Ensures all `AttendanceStatus` values appear in the output, even if zero.
        """
        active_ids = set(active_ids) if active_ids else None
        ...
        for student_id, status in self._staged.items():
            ...
            pending[student_id] = status

        summary = Counter(pending.values())
    ...
    def preview_overlay(
    ...
    ) -> dict[str, AttendanceStatus]:
        """
        Generate a merged view of Gradebook + staged attendance data.

        Args:
            active_ids (Collection[str]): The student IDs to include in the overlay.
            gradebook_status_map (dict[str, AttendanceStatus]): Current Gradebook values.

        Returns:
            dict[str, AttendanceStatus]: The effective status for each student, preferring staged values if different from the Gradebook.

        Notes:
            - This method is primarily used for previewing post-commit results without actually applying the staged changes.
            - If a staged status is identical to the Gradebook status, the Gradebook value is used.
        """
        ...
        for student_id in active_ids:
            gradebook_status = gradebook_status_map.get(
                student_id, AttendanceStatus.UNMARKED
            )

            staged_status = self._staged.get(student_id, None)

            effective_map[student_id] = (
                gradebook_status
                if staged_status is None or staged_status == gradebook_status
                else staged_status
            )
